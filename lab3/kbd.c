...
void (kbc_ih)() {
    if(read_KBC_output(OUT_BUF, &scancode) != 0) {
        ...
        scancode = 0; //foi sugerido dar reset ao scancode
    }
}
...
-> ppt slide 15
-> importante saber sobre os bits da "command byte" do KBC, quando ativados:
...
int (kbd_restore)() {
    ...
    if (write_KBC_cmd(KBC_CMD_REG, KBC_READ_CMD) != 0) return 1; //avisamos primeiro que vamos fazer uma leitura (o comando é o KBC_READ_CMD e escrevemo-lo no KBC_CMD_REG)
    if (read_KBC_output(OUT_BUF, &commandByte) != 0) return 1;   //lemos

    //ativamos o bit 0 para ativar as interrupts do keyboard
    commandByte = commandByte | BIT(0);

    //escrevemos a nova configuração
    if (write_KBC_cmd(KBC_CMD_REG, KBC_WRITE_CMD) != 0) return 1; //avisamos da escrita
    if (write_KBC_cmd(KBC_ARGS, commandByte) != 0) return 1; //escrevemos
